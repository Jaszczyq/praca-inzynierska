...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($selectedCategory) {
            $events = Event::where('category_id', $selectedCategory)->whereDate('date', '>=', Carbon::now())->inRandomOrder()->take(5)->get();
        } else {
            $events = Event::whereDate('date', '>=', Carbon::now())->inRandomOrder()->take(5)->get();
        }
